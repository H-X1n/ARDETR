...
from .utils import bias_init_with_prob, linear_init, _get_clones, inverse_sigmoid
...
class AttnRes(nn.Module):
    def __init__(self, dim=256):
        ...
        constant_(self.proj.weight, 0.0)
    def forward(self, prev_outputs: List[torch.Tensor]):
        if len(prev_outputs) == 1:
            V = prev_outputs[0].unsqueeze(0)
        else:
            V = torch.stack(prev_outputs, dim=0)  # [L, B, T, D]
        K = self.norm(V)
        q = self.proj.weight.squeeze(0)  # [D]
        logits = torch.einsum('d, l b t d -> l b t', q, K)
...
class BlockAttnResDeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        attn_out = self.self_attn(q.transpose(0, 1), k.transpose(0, 1), x.transpose(0, 1), attn_mask=attn_mask)[
            0
        ].transpose(0, 1)
        ...
        cross_out = self.cross_attn(
            self.with_pos_embed(x, query_pos), refer_bbox.unsqueeze(2), feats, shapes, padding_mask
        )
        cross_out = self.dropout2(cross_out)
        x = self.norm2(x+cross_out)
        # =====  FFN =====
        mlp_out = self.linear2(self.dropout3(self.act(self.linear1(x))))
        mlp_out = self.dropout4(mlp_out)
        x = self.norm3(x+mlp_out)
        return x


class BlockAttnResDeformableTransformerDecoder(nn.Module):
    def __init__(self, hidden_dim: int, decoder_layer: nn.Module, num_layers: int, step:int,eval_idx: int = -1):
        super().__init__()
        ...
        self.attn_res = nn.ModuleList([AttnRes(hidden_dim) for _ in range(num_layers//step)])
        self.norm = nn.ModuleList([nn.LayerNorm(hidden_dim) for _ in range(num_layers//step)])
    def forward(
            self,
            prev_outputs: List[torch.Tensor],  # decoder embeddings
        ...
        output = prev_outputs[0]
        for i, layer in enumerate(self.layers):
            output = layer(output, refer_bbox, feats, shapes, padding_mask, attn_mask, pos_mlp(refer_bbox))
            if (i + 1) % self.step == 0:
                prev_outputs.append(output)
                output = self.attn_res[(i + 1) // self.step - 1](prev_outputs)
                output = self.norm[(i + 1) // self.step - 1](output)
...
class BlockAttnResRTDETRDecoder(nn.Module):
    ...
    def __init__(
        ...
        # self.input_proj = nn.ModuleList(Conv(x, hd, act=False) for x in ch)
        # Transformer module
        decoder_layer = BlockAttnResDeformableTransformerDecoderLayer(hd, nh, d_ffn, dropout, act, self.nl, ndp)
        self.decoder = BlockAttnResDeformableTransformerDecoder(hd, decoder_layer, ndl, self.step, eval_idx)
        # Denoising part
        self.denoising_class_embed = nn.Embedding(nc, hd)
        self.num_denoising = nd
        self.label_noise_ratio = label_noise_ratio
...
attnres
"""
class AttnResDeformableTransformerDecoderLayer(nn.Module):
    def __init__(
            self,
            d_model=256,
            n_heads=8,
        ...
        super().__init__()
        self.attn_res_attn = AttnRes(d_model)
        self.attn_res_cross = AttnRes(d_model)
        self.attn_res_mlp = AttnRes(d_model)
    ...
    def forward(
        ...
        x = self.attn_res_attn(prev_outputs)
        x = self.norm1(x)
        # =====  Cross-Attention =====
        cross_out = self.cross_attn(
            self.with_pos_embed(x, query_pos), refer_bbox.unsqueeze(2), feats, shapes, padding_mask
        ...
        cross_out = self.dropout2(cross_out)
        prev_outputs.append(cross_out)
        x = self.attn_res_cross(prev_outputs)
        x = self.norm2(x)
        # =====  FFN =====
        mlp_out = self.linear2(self.dropout3(self.act(self.linear1(x))))
        mlp_out = self.dropout4(mlp_out)
        prev_outputs.append(mlp_out)
        x = self.attn_res_mlp(prev_outputs)
        x = self.norm3(x)
        return x,prev_outputs



class AttnResDeformableTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        output=prev_outputs[0]
        for i, layer in enumerate(self.layers):
            output,prev_outputs = layer(output, prev_outputs, refer_bbox, feats, shapes, padding_mask, attn_mask, pos_mlp(refer_bbox))
            bbox = bbox_head[i](output)
            refined_bbox = torch.sigmoid(bbox + inverse_sigmoid(refer_bbox))
            if self.training:
                dec_cls.append(score_head[i](output))
...
# 4. 完整AttnResRTDETRDecoder
# ======================================
class AttnResRTDETRDecoder(nn.Module):
    export = False  # export mode
    shapes = []
    anchors = torch.empty(0)
    valid_mask = torch.empty(0)
    ...
    def __init__(
        ...
        # self.input_proj = nn.ModuleList(Conv(x, hd, act=False) for x in ch)
        # Transformer module
        decoder_layer = AttnResDeformableTransformerDecoderLayer(hd, nh, d_ffn, dropout, act, self.nl, ndp)
        self.decoder = AttnResDeformableTransformerDecoder(hd, decoder_layer, ndl, eval_idx)
        # Denoising part
        self.denoising_class_embed = nn.Embedding(nc, hd)
        self.num_denoising = nd
        self.label_noise_ratio = label_noise_ratio
